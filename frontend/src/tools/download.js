...
export const downloadImage = async (view, mapType) => {
    ...
    if (mapType === "cumulative") {
        string =
            getDateString(view.offsetStart) +
            " - " +
            getDateString(view.offset);
    } else {
...
const getDateString = (offset) => {
    return store.getters["ui/getDateByOffset"](
        offset,
        "EEEEEE d MMM yyyy",
        store.state.languages.current.iso_code
    );
};
