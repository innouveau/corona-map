...
const load = function () {
    let url =
        ...
        "data-dashboards/cases_ggd_agegroups.csv" +
        dateTool.getTimestamp();
    store.commit("ui/updateProperty", {
        key: "caseDataRequested",
        value: true,
    });
    d3.csv(url)
        .then((data) => {
...
const sortEntries = function (entries) {
    ...
    for (let ggdData of ggds) {
        let ggd = store.getters["ggds/getItemByProperty"](
            "ggd_code",
            ggdData.ggd_code,
            true
        );
        ...
        store.commit("ggds/updatePropertyOfItem", {
            item: ggd,
            property: "ageGroupReport",
            value: ggdData.report,
        });
    }
