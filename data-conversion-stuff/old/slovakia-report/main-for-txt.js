...
function readText(result) {
    let lines, linesWithData, startWith;
    startWith = ['Kraj Okres Obec', 'test poz', 'Tabuľka'];
    lines = result.split('\n');
    linesWithData = lines.filter(line => {
        function containsChunk(l) {

            for (let chunk of startWith) {
                if (l.indexOf(chunk) > -1) {
                    return true;
                }
            }
            return false;
        ...
        return !containsChunk(line) && isNaN(firstChar) ;
    });
    ...
    for (let line of linesWithData) {
        ...
        if (linesWithData.indexOf(line) === 1) {
            console.log(chunks);
        }
        if (chunks.length > 1) {
            title = chunks[2];
